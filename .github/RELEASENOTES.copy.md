...
Only `PullRequestHandler` actually runs a Linux fast lane job. Previously, enabling `linuxFastLane: true` for any other workflow (`CICD`, `CreateRelease`, ...) - including through a repo- or org-wide default with no per-workflow scoping - made `DetermineProjectsToBuild` route the project into a build dimension nothing consumed: no job ran, no error was raised, and the project silently never built. `DetermineProjectsToBuild` now folds those projects back into the standard Windows build dimension for every workflow except `PullRequestHandler`, so a broad `linuxFastLane` default can no longer make a project vanish from a build.
